...
class MNISTReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        h_1 = self.PReLU(self.input_fc(self.flatten(x)))
    ...
    def init_weights(self, cb=0.0, cw=1.0):
        ...
        init_linear_weights(self.input_fc, self.bias_on, cb, cw/self.input_fc.in_features)
        init_linear_weights(self.hidden_fc, self.bias_on, cb, cw/self.hidden_fc.in_features)
        init_linear_weights(self.output_fc, self.bias_on, cb, cw/self.output_fc.in_features)
