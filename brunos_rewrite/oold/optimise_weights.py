...
class ParameterProcessor:
    ...
    def save_delta_current(self, momentum, weight_decay = 0.0): #, eta, eta_scale = 1.0
        #eta_w = eta * eta_scale
        with torch.no_grad():
            for name, grad in self.grad_current.items():
                ...
                if delta is None or momentum <= 0.0:
                    ...
                    self.delta_current[name] = -grad- weight_decay*self.theta_current[name]
                else:
...
class NetLineStepProcessorAbstract:
    ...
    def eta_analytic_n2_onehot(self, eta_test, delta_pq, delta_qq, norm_pq, norm_qq):
        with torch.no_grad():
            ...
            eta_next = self.alpha*norm_pq*cos_phi*eta_test/(norm_qq + self.beta)
            if self.do_logging:
                logging.info("##cos(pp^qq)={}, norm_pq={}, norm_qq={}, eta_test={}, eta_next={}, alpha={}, beta={}"\
